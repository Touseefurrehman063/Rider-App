...
class LabInvestigationController extends GetxController implements GetxService {
  ...
  updatedistype(typevalue) {
    if (typevalue == 1 || typevalue == 2) {
      if (LabInvestigationController.i.appointments.isNotEmpty) {
        if (typevalue == 1) {
          TypeValue = "Amount";
          LabInvestigationController.i.appointments[0].discountType = "Amount";
        } else if (typevalue == 2) {
...
//   }
  ...
  updatefinalsubtotal1() {
    for (int i = 0; i < selectedLabTests.length; i++) {
      totalprice = selectedLabTests[i].price!;
    }
...
// DateTime selectedDate = DateTime.now();
  ...
  updatequantityprice() {
    dynamic price = selectedLabtest!.price!;
    dynamic quantity = int.parse(selectedLabtest!.quantity!);
    dynamic subtotal = price * quantity;
    selectedLabtest!.price = subtotal;
    update();
  }

  void updatequantityprice1() {
    for (int i = 0; i < selectedLabTests.length; i++) {
      if (selectedLabTests[i].price != null) {
        // Check if price is not null
        double price = selectedLabTests[i].price!;
        int quantity = selectedLabTests[i].quantity ??
            0; // Use 0 as default if quantity is null

        double subtotal = price * quantity;
        selectedLabTests[i].price = subtotal;
      }
    }

    update(); // Assuming update() triggers a UI update or some other functionality
  }

  double ttPrice = 0.0;
  calculateedittotal() {
    for (int i = 0; i < LabInvestigationController.i.appointments.length; i++) {
      ttPrice += LabInvestigationController.i.appointments[i].price;
    }
    update();
  }
  ...
  void updatetestlist() {
    ...
    for (int i = 0; i < LabInvestigationController.i.appointments.length; i++) {
      ...
      name += LabInvestigationController.i.appointments[i].subServiceName
          .toString();

      // Add comma if not the last item
      if (i < LabInvestigationController.i.appointments.length - 1) {
        name += ", ";
      }
    }
  ...
  removeLabTest(int index) {
    if (totalprice > 0.0) {
      totalprice = totalprice - selectedLabTests[index].price!;
      update();
    }
  ...
  getLabTests(pid) async {
    LabTestsModelHome result = await LabInvestigationRepo.getLabTests(pid);
  ...
  getserviceslist() async {
    ...
    LabTestsModel results =
        await LabInvestigationRepo.getservices(result[0].id, 8);
