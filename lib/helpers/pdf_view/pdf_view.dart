...
class _PdfViewerPageState extends State<PdfViewerPage> {
  ...
  void sharePdfFile(String filePath, String fileName) {
    Share.shareFiles([filePath], text: 'Sharing PDF file', subject: fileName);
  }

  Future<File> getFileFromUrl(String url, {name}) async {
    var fileName = name ?? 'INVOICE';
    if (name != null) {
      fileName = name;
    }
    try {
  ...
  void initState() {
    ...
    getFileFromUrl("$ip2/${widget.url}", name: "INVOICE").then((value) => {
          log('$ip2/${widget.url}'),
          setState(() {
            if (value != null) {
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                ...
                // For example:
                if (urlPDFPath != "") {
                  sharePdfFile(urlPDFPath, 'INVOICE');
                } else {
                  const ScaffoldMessenger(child: Text("PDF is not available"));
                }
              },
            ),
          ],
