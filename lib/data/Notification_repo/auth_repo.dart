...
class AuthRepo {
  Future<String> updatePatientProfile(pid) async {
    ...
    var body = {
      ...
      "CNICNumber": EditPatientController.i.nationalid.text,
    };
    try {
  ...
  // Future<List<PdfData>> getPdf() async {
  //   String url = '$ip/api/account/GetConsentFormList';
  //   Uri uri = Uri.parse(url);
  //   var body = {
  //     "start": "0",
  //     "length": "1000",
  ...
  //         data.map((json) => PdfData.fromJson(json)).toList();
  //     return pdflist;
  //   } else {
  //     throw Exception('Failed to fetch title details');
  //   }
  ...
  Future getPdf() async {
    final url = '$ip/api/account/GetConsentFormList';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['Status'] != null &&
            data['Status'] == 1 &&
            data['Data'] != null) {
          for (var pdfdata in data['Data']) {
            CustomContainerController.i.selectedpdflist
                .add(PdfData.fromJson(pdfdata));
            print("PDF Data: $data['Data']");
          }
